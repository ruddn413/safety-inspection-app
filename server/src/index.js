...
app.get('/api/laws', async (req, res) => {
    try {
        ...
        const [lawRes, admrulRes, noticeRes] = await Promise.all([
            axios_1.default.get(`https://www.law.go.kr/DRF/lawSearch.do?OC=${apiKey}&target=law&type=XML&query=${lawQuery}`, { httpsAgent, headers }),
            axios_1.default.get(`https://www.law.go.kr/DRF/lawSearch.do?OC=${apiKey}&target=admrul&type=XML&query=${admrulQuery}`, { httpsAgent, headers }),
            axios_1.default.get(legislativeNoticeUrl, { httpsAgent }).catch(e => { console.error('Notice API error:', e.message); return { data: '' }; })
        ]);
        // Parse XML responses
        const [lawParsed, admrulParsed, noticeParsed] = await Promise.all([
            (0, xml2js_1.parseStringPromise)(lawRes.data),
            (0, xml2js_1.parseStringPromise)(admrulRes.data),
            noticeRes.data ? (0, xml2js_1.parseStringPromise)(noticeRes.data).catch(() => ({})) : Promise.resolve({})
        ]);
        const combinedLaws = [];
        // Map Current Laws (현행 법령)
        if (lawParsed && lawParsed.LawSearch && lawParsed.LawSearch.law) {
            lawParsed.LawSearch.law.forEach((lawItem) => {
        ...
        if (admrulParsed && admrulParsed.AdmRulSearch && admrulParsed.AdmRulSearch.admrul) {
            admrulParsed.AdmRulSearch.admrul.forEach((lawItem) => {
        ...
        if (noticeParsed && noticeParsed.result && noticeParsed.result.list && noticeParsed.result.list[0] && noticeParsed.result.list[0].ApiList01Vo) {
            noticeParsed.result.list[0].ApiList01Vo.forEach((noticeItem) => {
